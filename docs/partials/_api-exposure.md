...
These APIs are **not authenticated** and [must not be publicly exposed](/apis/ethereum-beacon-node-api.mdx). Anyone who can reach them can query your node and change proposer settings such as the fee recipient, redirecting your block priority-fee ("tip") earnings to an attacker-controlled address.
