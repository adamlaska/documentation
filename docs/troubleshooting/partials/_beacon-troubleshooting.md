<table>
    ...
    <tr>
      ...
      <td>{"Peers will continuously disconnect and reconnect, so don't worry about "}<code>Peer disconnected</code>{' messages. If your beacon node is struggling to find peers, check connectivity with '}<a href='/manage-connections/configure-ports-and-firewalls.md'>Improve P2P connectivity</a>{', verify your genesis state and network flag against the '}<a href='/install-prysm/install-with-script.md'>Quickstart</a>{', and on Windows check time sync if peers keep dropping. See '}<a href='https://github.com/OffchainLabs/prysm/issues/13431#issuecomment-1891167396'>issue 13431</a>{' and '}<a href='https://github.com/OffchainLabs/prysm/issues/8144'>issue 8144</a>{'.'}</td>
    </tr>
    <tr>
