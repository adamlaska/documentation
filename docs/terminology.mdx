...
#### BLS key
Validators use [BLS](/learn/dev-concepts/bls-cryptography.mdx) key pairs to sign consensus messages. A validator public key is visible on staking explorers; its private key must remain secret. It is not an Ethereum execution address.
