...
While it's possible to optimize your client instance architecture for high-availability and redundancy, we encourage validators to **keep it simple**. Complex validator architectures run the risk of accidentally engaging in slashable behavior. This can result in slashing [<a href='#footnote-4'>4</a>], which is a far steeper price to pay than the occasional downtime penalty.
 ...
 - **Advanced**: Configure [Prometheus and Grafana](./monitoring-alerts-metrics/grafana-dashboard.mdx) to help you visualize real-time validator metrics.
...
Put simply: Ethereum gently discourages downtime with paper cuts. But it uses a ruthless banhammer to punish clones, and it doesn’t have any way to distinguish between accidental clones and real, malicious clones. So it’s best to keep it simple and expect some paper cuts.
 ...
 - **Essential**: If you migrate your validator client to another machine, don't migrate your keys without also [migrating your slashing protection history](/backup-and-migration/slashing-protection.mdx). If you lose your slashing protection history, we recommend waiting few epochs before starting your validator.
...
You’ll be managing two types of keys: validator keys and withdrawal keys. Prysm only needs access to your validator keys. You can learn more about this [here on the Ethereum blog](https://blog.ethereum.org/2020/05/21/keys/).
 ...
 - **Essential**: Review [Prysm’s security considerations](/learn/concepts/keys-wallets-accounts.mdx#security-considerations) before generating your wallet and keys.
 ...
 - **Advanced**: Use [Web3Signer](/manage-wallet/use-web3signer.mdx) to maintain separation between your keys and client software.
...
Ethereum and its client software are constantly improving. This constant change means that unexpected things may happen that require troubleshooting. 
 ...
 - **Essential**: Learn how to [inspect Prysm's performance](/monitoring-alerts-metrics/monitor-prysm.mdx).
 - **Essential**: Familiarize yourself with [Prysm's P2P connectivity guidance](/manage-connections/configure-ports-and-firewalls.mdx).
 - **Essential**: Review [the Prysm FAQ](/faq.mdx).
...
Migrating your validator from one machine to another is a delicate process that requires attention to detail. Migrating between machines significantly increases the risk of slashing.
 ...
 - **Essential**: Review [Prysm's migration guidance](/backup-and-migration/migrating-keys.mdx) for a detailed overview of the process and considerations.
...
<strong id='footnote-4'>4</strong>. See the <a href='/backup-and-migration/slashing-protection.mdx'>slashing protection guide</a> before moving validator keys.<br />
...
<strong id='footnote-7'>7</strong>. See the <a href='/backup-and-migration/slashing-protection.mdx'>slashing protection guide</a>. <br />
