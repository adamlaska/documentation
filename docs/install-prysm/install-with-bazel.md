...
To run a beacon node, you'll need access to an execution node. See [Configure an execution node](/configure-prysm/configure-an-execution-node.mdx) for detailed instructions if you don't already have an execution node configured.
