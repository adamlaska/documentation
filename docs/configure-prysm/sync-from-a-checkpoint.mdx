...
You can extend the retention period beyond the minimum using the `--blob-retention-epochs` flag. The node will exit with an error if you attempt to set a value less than 4,096 epochs. For more details on storage requirements and configuration, see [Blobs](/learn/concepts/blobs.mdx).
