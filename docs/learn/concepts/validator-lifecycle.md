...
## `ACTIVE` State
Once the activation epoch arrives, the validator is activated and assigned responsibilities, including [proposing](/terminology#proposal-propose-) or [attesting](/terminology#attestation-attest-) to blocks on the beacon chain. Validators receive rewards or penalties on their initial deposit based on their overall performance. If a validator’s balance drops below 16 `ETH` (typically due to inactivity), it will be ejected. Ejections are treated the same as voluntary exits.
