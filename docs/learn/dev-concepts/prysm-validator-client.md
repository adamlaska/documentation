...
3. A validator is assigned to a shard either as a [proposer](../../terminology.md#proposal-propose) \(creator\) or [attester](../../terminology.md#attestation-attest) (voter).
4. The validator then has a ticker that works every slot (6 seconds). If the slot ticks at the validator's assigned slot, a beacon block is either [proposed](../../terminology.md#proposal-propose) or [attested](../../terminology.md#attestation-attest), depending on the assigned role.
...
A [block proposal](../../terminology.md#proposal-propose) must include several items to meet the minimum requirements for verification by the protocol. In chronological order, these steps are:

1. The [canonical head block](../../terminology.md#canonical-head-block) is fetched from the beacon node and its root is used as the parent root of the new block.
2. All pending deposits which have not yet been included in the chain are fetched.
3. [ETH1](../../terminology.md#eth1) data used to vote on deposit objects is fetched.
4. All pending slashings and [validator](../../terminology.md#validator) voluntary exits are fetched.
...
7. All pending [attestations](../../terminology.md#attestation-attest) from the beacon node are fetched.
8. The block object is constructed by packaging the above items into a block data structure.
9. The state root hash is computed, signing the block with the [validator](../../terminology.md#validator)'s private key.
10. The block is [proposed](../../terminology.md#proposal-propose) by sending it to the beacon node via [gRPC](/apis/prysm-public-api.md).

### Attesting to a Beacon Block

[Attesting](../../terminology.md#attestation-attest) to a block is a similar process to proposing, albeit with a few slightly different steps:
...
4. The attestation key is then signed with a [validator](../../terminology.md#validator)'s private key.
