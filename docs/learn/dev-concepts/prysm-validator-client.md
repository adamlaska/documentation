...
## How does it work?
A validator instance is permitted to begin participating in the network once 32 `ETH` has been locked in a [validator deposit contract](/learn/dev-concepts/validator-deposit-contract.md). Validators are tasked with correctly [proposing](../../terminology.mdx#proposal-propose) or [attesting](../../terminology.mdx#attestation-attest) to blocks on the beacon chain and receive rewards or penalties on their initial deposit based on their overall performance.
...
3. A validator is assigned to a shard either as a [proposer](../../terminology.mdx#proposal-propose) \(creator\) or [attester](../../terminology.mdx#attestation-attest) (voter).
4. The validator then has a ticker that works every slot (6 seconds). If the slot ticks at the validator's assigned slot, a beacon block is either [proposed](../../terminology.mdx#proposal-propose) or [attested](../../terminology.mdx#attestation-attest), depending on the assigned role.
...
A [block proposal](../../terminology.mdx#proposal-propose) must include several items to meet the minimum requirements for verification by the protocol. In chronological order, these steps are:

1. The [canonical head block](../../terminology.mdx#canonical-head-block) is fetched from the beacon node and its root is used as the parent root of the new block.
2. All pending deposits which have not yet been included in the chain are fetched.
3. [ETH1](../../terminology.mdx#eth1) data used to vote on deposit objects is fetched.
4. All pending slashings and [validator](../../terminology.mdx#validator) voluntary exits are fetched.
5. The latest fork data from the beacon chain is retrieved.
6. A randao reveal is generated by [BLS](/learn/dev-concepts/bls-cryptography.mdx) signing the epoch.
...
[Attesting](../../terminology.mdx#attestation-attest) to a block is a similar process to proposing, albeit with a few slightly different steps:
