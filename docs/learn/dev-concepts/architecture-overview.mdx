...
* A [**beacon node**](/learn/dev-concepts/prysm-beacon-node.mdx) which powers the beacon chain at the core of Ethereum consensus.
* A [**validator client**](/learn/dev-concepts/prysm-validator-client.mdx) connects to the beacon node and manages staking keypairs.
* A [**public RPC server**](/apis/prysm-public-api.mdx) to request data about network state, blocks, validators etc.
* A [**persistent key-value store**](/learn/dev-concepts/boltdb-database.mdx) in the form of a database ([BoltDB](/learn/dev-concepts/boltdb-database.mdx)).
* A [**P2P networking framework and server**](/learn/dev-concepts/p2p-networking.mdx) to connect with other beacon nodes.
...
The beacon chain runs through a distributed network of nodes known as [beacon nodes](/learn/dev-concepts/prysm-beacon-node.mdx). Participants who want to run a beacon node and help secure the network can stake 32 `ETH` to have their [validator client](/learn/dev-concepts/prysm-validator-client.mdx) join the pool of validators, who are responsible for [proposing](../../terminology.mdx#proposal-propose) and [attesting](../../terminology.mdx#attestation-attest) to new blocks on the beacon chain. This deposit does not come out of nowhere; validators transfer Ether from the [execution](../../terminology.mdx#execution-layer) chain to the system using a [validator deposit contract](/learn/dev-concepts/validator-deposit-contract.md).
