...
The beacon chain runs through a distributed network of nodes known as [beacon nodes](/learn/dev-concepts/prysm-beacon-node.mdx). Participants who want to run a beacon node and help secure the network can stake 32 `ETH` to have their [validator client](/learn/dev-concepts/prysm-validator-client.md) join the pool of validators, who are responsible for [proposing](../../terminology.mdx#proposal-propose) and [attesting](../../terminology.mdx#attestation-attest) to new blocks on the beacon chain. This deposit does not come out of nowhere; validators transfer Ether from the [execution](../../terminology.mdx#execution-layer) chain to the system using a [validator deposit contract](/learn/dev-concepts/validator-deposit-contract.md).
