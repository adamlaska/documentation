...
This last rule, which is the only rule from the consensus layer side preventing a node from importing a block, is to mitigate an attack known as the *forkchoice poisoning attack* which we will cover below in section [2.4](#2-4-forkchoice-poisoning).
...
Notice that this may happen during optimistic syncing of the merge block, precisely in the situation that the [forkchoice poisoning](#2-4-forkchoice-poisoning) attack described. In this case, even with the provision of `SAFE_SLOTS_TO_IMPORT_OPTIMISTICALLY`, we would be deadlocked.
